...
class ZoroAcados:
    def __init__(
        ...
        if h_tightening_jac_sig_fun is None:
            ...
            h_jac_x_fun, h_tighten_fun, h_tighten_jac_x_fun, h_tighten_jac_sig_fun = (
        ...
        self.p_hat_model_with_Pvec[0, : int((self.nx + 1) * self.nx / 2)] = (
            self.P_bar_all_vec[0]
        )
    ...
    def solve(self, tol_nlp=1e-6, n_iter_max=30):
        ...
        for i in range(n_iter_max):
            ...
            for stage in range(self.N):
                ...
                self.P_bar_all[stage + 1] = P_propagation(
                    self.P_bar_all[stage],
                    A_total,
                    self.B,
                    self.Sigma_W + np.diag(self.var[stage, :]),
                )
                ...
                self.p_hat_model_with_Pvec[
                    stage + 1, : int((self.nx + 1) * self.nx / 2)
                ] = self.P_bar_all_vec[stage + 1]

                self.solve_stats["timings"]["propagate_covar"][i] += (
                    perf_counter() - time_propagate_covar
                )
                ...
                p_sig = np.hstack((self.P_bar_all_vec[stage], self.p_hat_model[i, :]))
                htj_sig = self.h_tightening_jac_sig_fun(
                    self.x_hat_all[stage, :], self.u_hat_all[stage, :], p_sig
                )
                self.solve_stats["timings"]["get_backoffs_htj_sig"][i] += (
                    perf_counter() - time_get_backoffs_htj_sig
                )

                time_get_backoffs_htj_sig_matmul = perf_counter()
                htj_sig_matmul = htj_sig @ dP_bar_vec
                self.solve_stats["timings"]["get_backoffs_htj_sig_matmul"][i] += (
                    perf_counter() - time_get_backoffs_htj_sig_matmul
                )
                ...
                self.solve_stats["timings"]["get_backoffs_add"][i] = (
                    time_now - time_get_backoffs_add
                )
                self.solve_stats["timings"]["get_backoffs"][i] += (
                    time_now - time_get_backoffs
                )
                ...
                self.solve_stats["timings"]["set_tightening"][i] += (
                    perf_counter() - time_set_tightening
                )
