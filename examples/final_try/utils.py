...
def add_plot_trajectory(ax, tube_data: EllipsoidTubeData2D, color_fun=plt.cm.Blues, prob_tighten=1, **plot_args,):
...
def visualize_inverted_pendulum(X_sim, U_sim, time_vec, dt, REF=None, save_path=None, fps=30):
    """
    Animate (and optionally save) an inverted pendulum on a cart.
    ...
    - save_path: str or None, if given saves animation to this file
    ...
    def update_frame(k):
        ...
        return (cart_patch, wheel1, wheel2, pendulum_line, cart_ref_marker, timestamp)

    interval_ms = int(1000 * dt)

    ani = animation.FuncAnimation(fig, update_frame, frames=T, interval=interval_ms, blit=False)
...
def load_gp_data_from_csv(sim_file, n_outputs):
    ...
    - n_outputs (int): Number of columns containing gp targets.
...
def normalize_csv(csv_path):
    '''
    Scale training data (both inputs and outputs) by their mean and standard deviation
    '''
    # path to your CSV
    normalized_csv_path = "nominal_normalized.csv"
    ...
    df_scaled.to_csv(normalized_csv_path, index=False)

    # save the scaler parameters (mean & scale)
    scaler_params = {
...
def train_gp_model(gp_model, torch_seed=None, training_iterations=200, learning_rate=0.1):
    ...
    for i in range(training_iterations):
        ...
        output = (gp_model(train_x))   # !!!!! REMOVED LIKELIHOOD FROM HERE
...
def max_abs_scale(csv_path):
    ...
    # find the maximum absolute value in each of the last two columns
    max_abs = df[cols_to_scale].abs().max()

    print("Value of max_abs is: ", max_abs)

    # scale each value by dividing by the max absolute value
    df_scaled = df.copy()
    df_scaled[cols_to_scale] = df[cols_to_scale] / max_abs
...
def remove_first_column(csv_path):
    """
    Removes the first column and its label from a CSV file.

    Args:
        csv_path (str): Path to the CSV file.

    Returns:
        pd.DataFrame: DataFrame without the first column.
    """
    df = pd.read_csv(csv_path)
