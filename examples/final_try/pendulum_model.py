...
def export_pendulum_ode_model_actuation(dT, black_box = False) -> AcadosModel:
    '''
    Definition of cart pendulum system with actuation dynamic of first order. Both continuous and discrete dynamics
    '''
    model_name = 'pendulum_actuation'
...
def export_pendulum_ode_model(dT) -> AcadosModel:
    '''
    Definition of the model for the cart pendulum. Both continuous and discrete dynamics.
    Includes the whole dynamics of the system.

    '''
    model_name = 'pendulum_real'
...
def export_pendulum_ode_model_double_delay(dT, black_box=False) -> AcadosModel:
    ...
    '''
    model_name = 'pendulum_double_delay'
    ode_flag = 0 if black_box else 1
...
def export_discrete_pendulum_ode_model(dT, black_box = False) -> AcadosModel:
    '''
    Define model for the pendulum with both continuous and discrete dynamics. 
    Dynamics of the velocities is disabled
    '''

    model_name = 'pendulum_no_dyn'
    if black_box:
    ...
    f_expl = vertcat(
        x3,
        x4,
        0,#*(-m * p * sin_theta * x4**2 + m * g * cos_theta * sin_theta + u) / denominator,
        0,#*(-m * p * cos_theta * sin_theta * x4**2 + u * cos_theta + (m_cart + m) * g * sin_theta) / (p * denominator),
    )
...
def export_discrete_integrator(model):
    ...
    '''
    Export the discrete dynamics of the model 
    '''
...
def discrete_pendulum_ode_model_with_past_states(dt, black_box=False) -> AcadosModel:
    '''
    Discrete dynamics with past states
    '''
    ...
    full_discrete = vertcat(
        xf,    # current state update (4 states), based on RK4 with actual control input
...
def export_ocp_cartpendulum(N, T, model, integrator_type, Q, R, Qe, only_lower_bounds=False, **model_kwargs):
    ...
    '''
    Export AcadosOcp for the standard cart pendulum model without any delay, possibility to choose the integrator type.
    '''
    # Limiti input
    lb_u = -50.0
    ub_u = 50.0
...
#def export_ocp_cartpendulum(N, T, model, integrator_type, Q, R, Qe, only_lower_bounds=False, **model_kwargs):
#    '''
#    Export AcadosOcp for the standard cart-pendulum model with configurable integrator type.
#    '''
#
#    # input bounds
#    lb_u = -50.0
...
#    ocp.dims.nx = nx
#    ocp.dims.nu = nu
#    ocp.dims.np = model.p.shape[0] if hasattr(model, "p") else 0
#    ocp.dims.ny = ny
#    ocp.dims.ny_e = ny_e
#    ocp.dims.ny_0 = ny
#    ocp.dims.N = N
...
#        ocp.solver_options.sim_method_newton_tol = 1e-6  # instead of 0
#
#    return ocp
